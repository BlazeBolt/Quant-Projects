...
def backtest_strategy(prices, short_period=10, long_period=30, initial_capital=10000, stop_loss_percent=5, use_ema=False, ma_function=None):
    """
    ...
    #calculate moving averages based on user choice
    if ma_function:
    ...
    # We can only start trading after both MAs have established values
    ...
    while current_day < len(prices)-1: # Leave room for the next day
        ...
        next_day_price = prices[current_day + 1] # For trade execution next day
        ...
        if current_day >= long_period - 1:
            ...
            # If we have data for yestderday as well, check for crossovers
            if current_day > long_period - 1:
                ...
                # Execute buy signal if not in position
                if buy_signal and not in_position:
                    ...
                    profit_loss_percent = (profit_loss / (entry_price * shares)) * 100
    ...
    # Calculate final portfolio value at the end
